...
void beep() {
  // active-low buzzer: HIGH = idle, LOW = beep
...
void setup() {
  ...
  beep();  // startup beep
  if (Serial) Serial.println("[SYSTEM] I2C emulator slave ready at 0x42");
}
...
void loop() {
  ...
  if (pendingClick) {
    ...
    click();
    eventFlags |= 0x04;
    if (Serial) Serial.println("[CMD] click");
  }
  ...
  if (currentPos < targetPos) {
    ...
    eventFlags |= 0x01;
    if (Serial) { Serial.print("[CMD] CW x"); Serial.print(steps); Serial.print(" -> pos="); Serial.println(currentPos); }
  } else if (currentPos > targetPos) {
    int steps = currentPos - targetPos;
    stepCCW(steps);
    currentPos = targetPos;
    eventFlags |= 0x02;
    if (Serial) { Serial.print("[CMD] CCW x"); Serial.print(steps); Serial.print(" -> pos="); Serial.println(currentPos); }
  }
  ...
  if (buz != lastBuz) {
    lastBuz = buz;
    if (Serial) { Serial.print("[BUZ] "); Serial.println(buz ? "ON" : "OFF"); }
  }

  // UART control (same commands as spoofer for direct testing)
  if (Serial && Serial.available()) {
    ...
    if (cmd == "ON" || cmd == "OFF") {
      ...
      beep();
      int steps = 1;
      if (cmd.length() > 3) steps = cmd.substring(3).toInt();
      ...
      beep();
      int steps = 1;
      if (cmd.length() > 4) steps = cmd.substring(4).toInt();
      ...
      Serial.println(isCooktopOn() ? "ON" : "OFF");

    } else {
      Serial.println("[ERR] Unknown command. Use: ON, OFF, CW <n>, CCW <n>, STATUS");
    }
  }
}
